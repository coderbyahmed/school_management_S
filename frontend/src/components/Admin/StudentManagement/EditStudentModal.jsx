...
const EditStudentModal = ({ student, isOpen, onClose, onSave }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={t('edit') + ' ' + t('student')} maxWidth="max-w-2xl">
      ...
      <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-3 pt-4 mt-4 border-t border-gray-200 dark:border-gray-700">
        <Button variant="secondary" onClick={onClose} disabled={loading} className="sm:w-auto">{t('cancel')}</Button>
        <Button variant="primary" onClick={handleSave} loading={loading} className="sm:w-auto">{t('update')} {t('student')}</Button>
      </div>
    </Modal>
  );
};
